...
def trouver_produit_similaire_StarVision(reference):
    ...
    return produit_similaire,prix_produit_similaire,remise_similaire,mots_reference



def trouver_produit_similaire_Seiko(reference,champ):
    print("EQUSEIKO SIMULAITE PROCEDUREALS")
    produit_equals = Seiko.objects.filter(reference=reference.upper())
    ...
    if produit_equals.exists():
        ...
        #mots_reference = reference.upper()  # Convertir la référence en majuscules et la diviser en mots
        max_mots_communs = 0  # Initialisez le nombre maximum de mots communs
        produit_similaire = None  # Initialisez la référence du produit similaire
        ...
        # Parcourir tous les produits dans la base de données
        #for produit in Seiko.objects.all():
    ...
    if request.method == 'POST' and request.FILES['pdf_file']:
        ...
        for command in commands[1:]:
            ...
            if any(jetstar in new_description for jetstar in JETSTAR_LIST):
                ...
                #break
                new_description = re.sub(r'\+ \d+', '', new_description)
                produit_catalogue,prix_catalogue,remise_catalogue,mots_reference=trouver_produit_similaire_StarVision(new_description)
                ...
                print(f"LOKING FOR  {new_description} IN SEIKO TABLE")
